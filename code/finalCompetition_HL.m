function[dataStore] = finalCompetition_HL(CreatePort,DistPort,TagPort,tagNum,maxTime)%,offset_x,offset_y)
%
...
nomore = 1;
dualenter = 0;
optwallfordetect = zeros(optwallsize,4);
for i=1:optwallsize
    optwallfordetect(i,:) = extendedge(optionalW(i,:), -0.1);
end
% global addwp;
% global originalwpsz;
...
for j=1:mapsize
    if j<=originalwpsz
        dw = plot([mapdata(j,1) mapdata(j,3)],[mapdata(j,2) mapdata(j,4)],'LineWidth',2,'Color','k','HandleVisibility','off')
    else
        ndw = plot([mapdata(j,1) mapdata(j,3)],[mapdata(j,2) mapdata(j,4)],'LineWidth',2,'Color','r','HandleVisibility','off')
    end
end
for e=1:beaconsize
...
while toc < Inf && finishAll~=1  % WITHIN SETTING TIME & LAST WAYPOINT IS NOT REACHED
    ...
    dataStore.robotestimate = [dataStore.robotestimate; robotestimate];
    ...
    if ~(spinsw <= 1)
        ...
        rbt = plot(partTraj(:,1),partTraj(:,2),'r');
    end
    % #####################SAMPLING END####################### %%
        ...
        elseif plan == 1
            ...
            if turnStart == 1
                ...
                if abs(turnSum) > pi
                    ...
                    SetFwdVelAngVelCreate(CreatePort, 0, 0);
                    stop = 1;
                    if realWall == 1 % check bump sensor again
                        backStart = 1;
                        turnStart = 0;
                        backSum = 0;
                        turnSum = 0;
    ...
    elseif spinsw == 3
        ...
        if spinAgain == 1
            ...
            stop = 0;
            spinSum = spinSum + dataStore.odometry(end,3);
            noiseprofile = [sqrt(0.00005) sqrt(0.00005) sqrt(0.05)];
            DRweight = 0;
            if abs(spinSum) >= 0.1*pi % 360 degree spinning
                spinDone = 1;
            end
            ...
            spinSum = spinSum + dataStore.odometry(end,3);
            if caliboneshot == 1 || (dataStore.timebeacon(end,1) ~= -1) %%&& abs(dataStore.timebeacon(end,3)) <= 0.15
                caliboneshot = 1;
                noiseprofile = [sqrt(0.0001) sqrt(0.0001) sqrt(0.01)];
                inititer = inititer+1;
                if startTimer == 0  % start counting staring sec
                    %                     disp("stopped and I see beacon!+")
                    DRweight = 0;
...
for i = 1:size(dataStore.visitedwp,1)
   wpv = plot(dataStore.visitedwp(i,1),dataStore.visitedwp(i,2),'bp','MarkerSize',10,'MarkerFaceColor','r');    % waypoints visited
end
legend([wpv rbt dw ndw],'Visited Waypoints','Robot Trajectory','Exist Wall','Not Determined Wall');

figure(2)
hold on
% rbt = plot(dataStore.robotestimate(:,1),dataStore.robotestimate(:,2),'b','LineWidth',1.5);    % robot's trajectory
% for i = 1:size(dataStore.visitedwp,1)
%    wpv = plot(dataStore.visitedwp(i,1),dataStore.visitedwp(i,2),'rp');    % waypoints visited
% end
% for j=1:size(dataStore.finalmap,1)
%     fm = plot([dataStore.finalmap(j,1) dataStore.finalmap(j,3)],[dataStore.finalmap(j,2) dataStore.finalmap(j,4)],'LineWidth',2,'Color','k','HandleVisibility','off');    %final map
% end
plot(dataStore.truthPose(:,2),dataStore.truthPose(:,3),'b','LineWidth',1.5); 
axis equal
