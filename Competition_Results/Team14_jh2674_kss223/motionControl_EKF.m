function[dataStore] = motionControl_EKF(CreatePort,DistPort,TagPort,tagNum,maxTime)
...
while toc < maxTime
    ...
    if initsw == 0
        ...
        %         dataStore.ekfSigma = [4 0 0;0 4 0;0 0 0.02];
        
        %5a
%         R = 0.01*eye(3);
%         Q = 0.001*eye(9); %for 9 point depth
        
        %5b
                R = 0.01*eye(3);
                Q = 0.1*eye(9); %for 9 point depth
        
        %         Q = 0.001*eye(3); %for GPS
        initsw = 1;
    ...
    elseif initsw == 1
        ...
        %         h_handle = @(mutbar) hGPS(mutbar);
        %         Hjac_handle = @(mutbar) HjacGPS();
