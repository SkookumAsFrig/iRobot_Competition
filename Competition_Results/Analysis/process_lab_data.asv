...
for i=1:length(dataStore.rsdepth(:,1))-1
    clf
    disp(i)
    hold on
    currtime = dataStore.rsdepth(i,1);
    [~,I] = min(abs(truthLog.truthPose(:,1) - 4.1 - currtime));
    for j=1:l
    ...
    partTraj = dataStore.robotestimate(1:i,:);
    
    f=plot(truthLog.truthPose(1:I,2),truthLog.truthPose(1:I,3),'b');
    g=plot(partTraj(:,1),partTraj(:,2),'r');
    
    h=drawparticlestar_red(xpartmean,ypartmean,tpartmean);
    k=drawparticlestar(truthLog.truthPose(I,2),truthLog.truthPose(I,3),truthLog.truthPose(I,4));
    ...
    if beacnumb == 0
        ...
        for j=1:beacnumb
            beacpoint = robot2global(truthLog.truthPose(I,2:4),dataStore.timebeacon(i,(j-1)*3+2:(j-1)*3+3));
    ...
    for j=1:9
        tempglob(j,:) = robot2global(truthLog.truthPose(I,2:4),[depthXY(1,j) depthXY(2,j)]);
    end
